...
| `recycle` | `RecyclePolicy()` — 10 min | **On by default.** At `maxAge` (`Generated.sessionMaxAgeMs`) the model closes both links itself — the stream is `.closed`, not `.lost` — raises `sessionRecycled(age:)` and dials again at once, keeping the handle, the streams and the tree, so a client reads `.connected → .reconnecting(attempt: 1) → .connected`. A device held to one session for hours has been seen to stop serving and flash red (docs/11). A reopen that fails falls through to `reconnect.stream`, counted from attempt two, or `.disconnected`. `nil` holds one session open for as long as it lasts. |
